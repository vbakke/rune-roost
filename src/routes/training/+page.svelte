<script lang="ts">
	...
	import { AsymCaesarRot } from '$lib/model/asym/AsymCaesarRot.ts';
	import { SymCaesar } from '$lib/model/sym/SymCaesar.ts';
    import { CaesarHash } from '$lib/model/hash/CaesarHash.ts';
	...
	// 2. Derived MessageModel (auto-creates when userInput changes)
	let message = $derived(new Message(userInput));

	// 3. Selectable encryption algorithms (stateless)
	let symmetricCipher = $state<SymmetricEncryption>(new SymCaesar());
	let asymmetricCipher = $state<AsymmetricEncryption>(new AsymCaesarRot());

	// 4. Keys stored in component (generated by encryption classes)
	let symmetricKey = $state(symmetricCipher.generateKey()); 
	let asymmetricKeyPair = $state(asymmetricCipher.generateKeyPair());

	// 5. Derived encrypted results (auto-update when message changes)
	...
	function regenerateKeysOnDiceRoll(func: () => void) {
		...
		const interval = setInterval(() => {
			...
			if (counter <= 0) {
				func();
				console.log(`${(performance.now()/1000).toFixed(3)}s: Final: ${asymmetricKeyPair.publicKey}`);
				clearInterval(interval);
			}
			else if (counter > 2) {
				func();
				console.log(`${(performance.now()/1000).toFixed(3)}s: Generated key: ${asymmetricKeyPair.publicKey}`);
			}
		}, 150);
	}
		...
		<div class="encryption-flow">
			...
			<div class="lesson">
				...
				<a href="/lær/asymmetric" class="lesson-button">
					...
					<div class="key">
						<div class="key-icon gold">🔑</div>
						<div class="key-value">{asymmetricKeyPair.privateKey.plain}</div>
					</div>
					<span>Decrypt</span>
				</a>
			</div>
